...
class TestFrameworkInfo(TypedDict):
    type: str
    framework: Optional[str]
    cmd: Optional[List[str]]
    coverage_cmd: Optional[List[str]]


def detect_test_framework(project_path: Path) -> TestFrameworkInfo:
    """Detect test framework and commands."""
    result: TestFrameworkInfo = {
        "type": "unknown",
        "framework": None,
        "cmd": None,
        "coverage_cmd": None
...
def run_tests(cmd: List[str], cwd: Path) -> RunTestResult:
