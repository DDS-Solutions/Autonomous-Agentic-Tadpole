...
from typing import List, Set, Dict, Any, TypedDict, Optional, Union

class UXAuditReport(TypedDict):
    files_checked: int
    issues: List[str]
    warnings: List[str]
    passed_checks: int
...
class UXAuditor:
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'coverage', 'playwright-report', '.gemini'}]
            for file in files:
                if Path(file).suffix in extensions:
                    self.audit_file(os.path.join(root, file))

    def get_report(self) -> UXAuditReport:
        return {
