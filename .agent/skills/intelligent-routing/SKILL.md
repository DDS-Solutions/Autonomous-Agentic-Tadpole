...
| **System Audit**    | "audit", "blast radius", "nexus", "openapi"| `nexus-engineer`                            | ✅ YES       |
...
> [!IMPORTANT]
> **The Orchestrator Protocol (Ask First)**: When routing to the `orchestrator` (due to complexity or new features), the routing agent must not invoke sub-agents immediately. It **must** first propose a high-level architecture/implementation plan to the user and await explicit confirmation.
...
| --------------- | ------------------------------------------ | ----------------------- |
| **Audit**       | audit, blast radius, nexus, openapi        | `nexus-engineer`        |
...
When keywords map to multiple potential domains or agents (e.g., `auth` mapping to both `Security` and `Authentication` sub-domains):
1. **Specialization Priority**: The most specialized agent (e.g., `security-auditor`) takes precedence over a generalist agent (e.g., `backend-specialist`), unless both are explicitly co-invoked.
2. **Multi-Domain Escalation**: If the ambiguous keywords span different system layers (e.g., frontend vs database), escalate to the `orchestrator` to coordinate the implementation.

### Scaling the Matrix

To prevent `SKILL.md` from becoming bloated as new agents and domains are added:
- Keep core rules and high-level routing categories in `SKILL.md`.
- Maintain a separate `REGISTRY.md` file in the skill directory containing the full, expanded list of keyword-to-agent mappings, allowing the routing matrix to remain clean and maintainable.
