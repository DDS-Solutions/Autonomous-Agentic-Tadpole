...
### AI Assist Note
**🎨 UX Auditor**: Conducts deep psychological and aesthetic analysis of frontend components. 
Validates compliance with Hick's Law, Fitts' Law, and the critical "Purple Ban" to ensure a premium, sovereign user experience.

### 🔍 Debugging & Observability
- **Failure Path**: Aesthetic deviation from the "Neural Glass" design tokens or violation of the "Purple Ban".
- **Telemetry Link**: Search `[ux_audit]` in system logs.
"""
...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Use precise HTML element matching to avoid false positives from CSS/docstrings
        has_form = bool(re.search(r'<form[\s>]|<input[\s>]|type=["\']password|credit[-\s]card|checkout|payment', content, re.IGNORECASE))
        complex_elements = len(re.findall(r'<input[\s>]|<select[\s>]|<textarea[\s>]|<option[\s>]', content, re.IGNORECASE))

        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law — threshold 9 to account for grouped/hierarchical sidebar nav sections
        # in dashboard apps where sub-items are visually distinct from top-level items.
        nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', content, re.IGNORECASE))
        if nav_items > 9:
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} nav items (Max 9)")
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        skip_dirs = {'node_modules', '.git', 'dist', 'build', '.next', 'coverage',
                     '__pycache__', '.vite', 'target', '.vitepress', '.agent', 'docs',
...
def main():
    print("[ux_audit] Initializing psychological and aesthetic UX audit...")
