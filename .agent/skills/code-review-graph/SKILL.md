...
| `OKF (Open Knowledge Format)` | Double-Graph alignment: combines technical symbol dependencies (code-review-graph) with business logic playbooks (OKF) |

### Integration with OKF (Open Knowledge Format)

Tadpole OS features a native Open Knowledge Format (OKF) memory store. AI agents should combine `code-review-graph` with OKF:
1. **Double-Graph Context**: Query `code-review-graph` to extract the structural code dependency path, and query `OKF` to retrieve the relevant architectural playbooks, security models, and compliance rules.
2. **Wiki Ingestion**: Exported codebase wikis (generated via `code-review-graph wiki`) can be converted into markdown files and registered in the OKF memory store, making technical architecture documentation semantically searchable by the swarm.
...
3. Query OKF playbooks for domain-specific security and design constraints
4. Context compression summarizes completed phases
5. Memory system saves key decisions for next session
6. Result: minimum tokens, maximum quality
```
