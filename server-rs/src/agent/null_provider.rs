...
impl LlmProvider for NullProvider {
    async fn generate(
        ...
        // 🛡️ [H25: Integrity Guard] Never forge fake mission runs or spoofed audit reports in NullProvider
        let text_response = format!(
            "[DEGRADED: {}] Agent '{}' has no configured provider. \
             Please configure a valid LLM provider and API key in Settings.",
            self.reason.as_str(),
            self.agent_id
        );

        Ok((text_response, Vec::new(), None))
    }
